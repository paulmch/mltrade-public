...
ndx = (s3.Object("REDACTED_BUCKET", "^NDX.csv").get()['Body'].read().decode('utf-8') )
analysisfile = open("/tmp/^NDX.csv", "w+")
analysisfile.write(ndx)
analysisfile.close()


def load_models(structure, model):
    tmp = (s3.Object("REDACTED_BUCKET", "reinfmodels/{}".format(structure)).get()['Body'].read().decode('utf-8') )
    analysisfile = open("/tmp/"+structure, "w+")
    analysisfile.write(tmp)
    analysisfile.close()
    
    
    tmp = (s3.Object("REDACTED_BUCKET", "reinfmodels/{}.index".format(model)).get()['Body'].read()  )
    analysisfile = open("/tmp/{}.index".format(model), 'wb')
    analysisfile.write(tmp)
    analysisfile.close()
    
    tmp = (s3.Object("REDACTED_BUCKET", "reinfmodels/{}.data-00000-of-00002".format(model)).get()['Body'].read()  )
    analysisfile = open("/tmp/{}.data-00000-of-00002".format(model), 'wb')
    analysisfile.write(tmp)
    analysisfile.close()
    
    tmp = (s3.Object("REDACTED_BUCKET", "reinfmodels/{}.data-00001-of-00002".format(model)).get()['Body'].read()  )
...
class stock(gym.Env):
    ...
    def __init__(self, df,train=True, stop = 0.71):
        super(stock, self).__init__()
        
        if train:
            #366
            self.df = df.iloc[0: 700].values
        else:
            self.df = df.values
        self.tmp = []
        self.timestep = 1
        ...
        self.action_space = spaces.Discrete(n_actions_1)
        
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(1,44), dtype=np.float32)
    ...
    def collect_data(self):
        ...
        for i in range(index,index+1):
            open_act = (values[i-1][1] - values[i][1]) / values[i-1][1]
            high_act = (values[i-1][2] - values[i][2]) / values[i-1][2]
            low_act = (values[i-1][3] - values[i][3]) / values[i-1][3]
            close_act = (values[i-1][4] - values[i][4]) / values[i-1][4]
    
          
            
            open_actual = (values[i][1] - values[i+1][1]) /values[i+1][1]
            high_actual = (values[i][2] - values[i+1][1]) /values[i+1][1]
            low_actual = (values[i][3] - values[i+1][1]) /values[i+1][1]
            close_actual = (values[i][4] - values[i+1][1]) /values[i+1][1]
            
            dayinfo = [float( i == values[i,5]) for i in range(0,5) ] +  [float( i == values[i,6]) for i in range(0,31)]
            tmp = [ open_act, high_act, low_act, close_act,open_actual,high_actual,low_actual, close_actual]
            tmp = tmp + dayinfo
            x.append(tmp)
    ...
    def buy_long(self,budget, x12,x13 ):
        ...
        knockoutprice = x12 * values[i+1][1]
        
        price_per_share = self.price(values[i+1][1], knockoutprice  )
    ...
    def buy_short(self,budget, x12,x13 ):
        ...
        price_per_share = self.price( knockoutprice, values[i+1][1]  )
        allocated_money = min(budget,x13 )
        budget = budget - allocated_money
        number_of_shares = int( (allocated_money-1)/ price_per_share   )
        allocated_money = allocated_money - number_of_shares * price_per_share -1
    ...
    def sell(self,budget, pos):
        ...
        if(len(pos) == 0 ):
            return budget,[]
        if pos[0] == "long":
            price_per_share =  self.price(values[i][4], pos[1]  )
            if self.price(values[i][2],pos[1] ) < pos[3] * self.price(values[i][1], pos[1]  ):
                price_per_share = pos[3] * self.price(values[i][1], pos[1]  )
            ...
            return budget, []
        if pos[0] == "short":
            price_per_share = self.price( pos[1],values[i][4]  )
            if self.price( pos[1],values[i][2]  )  <=  pos[3] * self.price( pos[1],values[i][1]  ):
                price_per_share = pos[3] * self.price( pos[1],values[i][1]  )
            if price_per_share == 0:
                return budget, []
            price_per_share = price_per_share - 0.01
    ...
    def take_action(self,step, action):
        ...
        return (self.budget - previous_budget) / min(previous_budget,Maximum_invest )*100
...
nsdaq = pd.read_csv(path,usecols=['Date','Open',"High","Low","Close"],  parse_dates=['Date'])
...
def build_model(parameters, structure,number_of_layers ,number_heads, number_of_head_layers,number_of_lstm_layers):
    ...
    if number_of_head_layers == 0:
        number_heads = 1
    reshape = tf.keras.layers.Reshape((1,44), input_shape=(1,) + env.observation_space.shape)(input_layer)
...
def handler(event, context):
    ...
    prediction =dqn.model.predict(tmp.reshape(1,1,1,44))[0]
    maxval = -1000
    j = 0
    ...
    s3.Object('REDACTED_BUCKET', 'agentprediction/{}/{}/{}/{}_{}.txt'.format(
        key[0],key[1],key[2],event["start_value"],event["modelnumber"]
