...
dax = (s3.Object("REDACTED_BUCKET", "^NDX.csv").get()['Body'].read().decode('utf-8') )
...
def download():
    ...
    period = 1659139200 + difference.days * 86400 
    
    url = "https://query1.finance.yahoo.com/v7/finance/download/%5ENDX?period1=496972800&period2={periodend}&interval=1d&events=history&includeAdjustedClose=true".format(periodend = period) 
...
def ready_csv():
    ...
    dax = dax.append(df2, ignore_index = True)
    dax.to_csv("/tmp/^NDX.csv", index = False)
...
def handler(event, context):
    ...
    if len(((feiertage['Date'].where(feiertage['Date'] == datestart)).dropna()).values) == 0 and today.weekday() <=4:
        ...
        analysisfile = open("/tmp/^NDX.csv", "r")
        data = analysisfile.read()
        analysisfile.close()
        s3.Object('REDACTED_BUCKET', '^NDX.csv').put(Body=data)
