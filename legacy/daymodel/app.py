...
def load_models(structure, model):
    tmp = (s3.Object("modelsreinf", "day/{}".format(structure)).get()['Body'].read().decode('utf-8') )
    analysisfile = open("/tmp/"+structure, "w+")
    analysisfile.write(tmp)
    analysisfile.close()
    
    
    tmp = (s3.Object("modelsreinf", "day/{}.index".format(model)).get()['Body'].read()  )
    analysisfile = open("/tmp/{}.index".format(model), 'wb')
    analysisfile.write(tmp)
    analysisfile.close()
    
    tmp = (s3.Object("modelsreinf", "day/{}.data-00000-of-00002".format(model)).get()['Body'].read()  )
    analysisfile = open("/tmp/{}.data-00000-of-00002".format(model), 'wb')
    analysisfile.write(tmp)
    analysisfile.close()
    
    tmp = (s3.Object("modelsreinf", "day/{}.data-00001-of-00002".format(model)).get()['Body'].read()  )
...
class stock(gym.Env):
    ...
    def __init__(self, df,train=True, stop = 0.8):
        ...
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(1,53), dtype=np.float32)
    ...
    def collect_data(self):
        ...
        for i in range(index,index+1):
            ...
            open_actual = (values[i+1][1] - values[i+1][9]) /values[i+1][9]
            high_actual = (values[i+1][2] - values[i+1][9]) /values[i+1][9]
            low_actual = (values[i+1][3] - values[i+1][9]) /values[i+1][9]
            close_actual = (values[i+1][4] - values[i+1][9]) /values[i+1][9]
            ...
            tmp = [open_pred, high_pred,low_pred, close_pred, open_act, high_act, low_act, close_act, open_close_dev
                 , high_close_dev, low_close_dev, close_close_dev, open_conf/Max_obsv,open_actual,high_actual,low_actual, close_actual]
            tmp = tmp + dayinfo
            x.append(tmp)
    ...
    def buy_long(self,budget, x12,x13 ):
        ...
        price_per_share = self.price(values[i+1][9], knockoutprice  )
    ...
    def buy_short(self,budget, x12,x13 ):
        ...
        price_per_share = self.price( knockoutprice, values[i+1][9]  )
        allocated_money = min(budget,x13 )
        budget = budget - allocated_money
        number_of_shares = int( (allocated_money-1)/ price_per_share   )
        allocated_money = allocated_money - number_of_shares * price_per_share -1
    ...
    def sell(self,budget, pos):
        ...
        if(len(pos) == 0 ):
            return budget,[]
        if pos[0] == "long":
            price_per_share =  self.price(values[i][12], pos[1]  )
            if self.price(values[i][11],pos[1] ) < pos[3] * self.price(values[i][9], pos[1]  ):
                price_per_share = pos[3] * self.price(values[i][9], pos[1]  )
            ...
            return budget, []
        if pos[0] == "short":
            price_per_share = self.price( pos[1],values[i][12]  )
            if self.price( pos[1],values[i][10]  )  <=  pos[3] * self.price( pos[1],values[i][9]  ):
                price_per_share = pos[3] * self.price( pos[1],values[i][9]  )
            if price_per_share == 0:
                return budget, []
            price_per_share = price_per_share - 0.01
...
def build_model(parameters, structure,number_of_layers ,number_heads, number_of_head_layers,number_of_lstm_layers):
    ...
    if number_of_head_layers == 0:
        number_heads = 1
    reshape = tf.keras.layers.Reshape((1,53), input_shape=(1,) + env.observation_space.shape)(input_layer)
...
def handler(event, context):
    ...
    prediction =dqn.model.predict(tmp.reshape(1,1,1,53))[0]
    maxval = -1000
    j = 0
    ...
    s3.Object('REDACTED_BUCKET', 'day/{}/{}/{}/{}_{}.txt'.format(
