...
def handler(event, context):
    ...
    predict =[ prediction(model,x_data_transformed[i],x_data_aux[i]) for i in range(startint,len(x_data_transformed)) ]
    stop5 = 0.71
    budget_list, direction_list = test_action(stop5,predict)
    del params["key"]
    model_parameter["performance"] = budget_list[len(budget_list)-1]
    s3.Object('REDACTED_BUCKET',"rawmodelday/modelstructure/{}".format(modelname)).put(Body=json.dumps(model_parameter))
